...
LLM_CONTEXT_BLOCK_ZH = (
    ...
    "      设计域实体默认为 **beso9 式三棱柱**（对齐 `examples/beso/beso9/BESO9.FCStd`）："
    "等边三角形底沿竖向拉伸，三顶角通高圆柱挖孔（可选中心孔），**不挖桩靴**以免底面台阶怪形；"
    "另可复制为 `{stem}-Compound.iges` 便于与主 IGES 并列扫描；**剖分仍使用** `01_design_domain.step`。"
    "若需整装配包围盒平板并挖柱/桩靴，设 `OC4_DESIGN_DOMAIN_ENVELOPE=hull` 或 `domain_envelope=hull_bbox`。\n"
    ...
    "B. 设计域「怎么画 / 怎么建几何」\n"
    "  - OC4：`build_oc4_design_domain_iges` 默认用 **三棱柱包络**（beso9：等边底 + 顶角通高挖孔，不挖桩靴）；"
    "可选 `OC4_DESIGN_DOMAIN_ENVELOPE=hull` 改为整装配包围盒 slab 并挖柱/桩靴；\n"
