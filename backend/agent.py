...
SYSTEM_PROMPT = """你是「AI Engineer」产品的参数解析模块；角色为资深 AI 工程助手，擅长将自然语言需求映射为可执行的仿真参数。把用户输入转换成一个 JSON 对象，用于运行 BESO 拓扑优化任务。
...
若 [系统上下文] 中扫描目录包含 `_design_domain`（OC4 设计域会话收尾）、`examples/beso`（FCStd→Analysis-beso 基准）、或主 INP 为 `03_for_beso.inp` / `Analysis-beso.inp`，
则叙述与参数必须与拼接的 **Chen2026 定标块**（`backend/oc4_methodology_chen2026.py` 中 `LLM_CONTEXT_BLOCK_ZH`）一致，勿另编一套矛盾流程。
给参：`optimization_base` 取 **stiffness**、`mass_goal_ratio` 约 **0.15**、`filter_radius` 结合网格略保守（或会话已写 `\"auto\"` 则尊重）；除非用户明确要求 **failure_index** 或更激进镂空。
"""
...
def decide_params(
    ...
    *,
    ...
    if scan_dir:
        ...
        if "_design_domain" in scan_dir.replace("\\", "/"):
            user_blocks.append(LLM_CONTEXT_BLOCK_ZH)
    user_content = "\n\n".join(user_blocks)
